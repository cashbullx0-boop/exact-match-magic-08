CREATE OR REPLACE FUNCTION public.referral_reward_cents(_ts timestamp with time zone DEFAULT now())
 RETURNS integer
 LANGUAGE sql
 IMMUTABLE
...
REVOKE EXECUTE ON FUNCTION public.admin_approve_deposit(uuid) FROM PUBLIC, anon, authenticated;
