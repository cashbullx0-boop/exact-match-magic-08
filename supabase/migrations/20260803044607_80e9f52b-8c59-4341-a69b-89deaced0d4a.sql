...
BEGIN
  ...
  -- FIXED: dedupe by referred user across ALL of their deposits, not just the earliest one.
  FOR rec IN
    SELECT p.id AS referred_id, p.referred_by
    ...
    WHERE p.referred_by IS NOT NULL
      ...
      AND NOT EXISTS (
        ...
        WHERE t.user_id = p.referred_by
          ...
          AND (
            t.related_id = p.id
            OR t.related_id IN (SELECT d3.id FROM public.deposits d3 WHERE d3.user_id = p.id)
          )
      )
    LIMIT 100
  LOOP
...
DECLARE v_user uuid := 'c2ba7b27-b24f-489c-a36e-4d67646f4565';
        v_dup uuid := '88aafb40-61d1-4542-b97e-cd359403fbf9';
BEGIN
