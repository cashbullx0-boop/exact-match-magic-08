...
BEGIN
  -- Whole-job mutex: if another run is already processing, skip this tick entirely.
  ...
  BEGIN
    ...
    LOOP
      ...
      -- Compute the new schedule deterministically from the trade's own clock,
      -- not from now(), so credits stay aligned and can't drift on retries.
      new_next := t.next_profit_at + make_interval(secs => cycle_seconds * cycles_missed);

      -- Conditional update: only one transaction can win this for a given (id, next_profit_at).
      -- If another worker already advanced next_profit_at, this updates 0 rows and we skip crediting.
      ...
      -- Secondary idempotency: refuse to insert a duplicate ledger row for the
      -- same trade + cycle window in case this function is invoked from another path.
      IF EXISTS (
