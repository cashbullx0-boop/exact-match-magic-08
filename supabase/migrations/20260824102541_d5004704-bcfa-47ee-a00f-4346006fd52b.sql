...
SET value = (value - 'pool_guard') || jsonb_build_object('auto_guard', true, 'max_payout_percent', COALESCE((value->>'max_payout_percent')::numeric, 60)),
    updated_at = now()
WHERE key = 'spinner_config';
...
BEGIN
  ...
  IF auto_guard THEN
    ...
    v_reward_cents := GREATEST(LEAST(v_reward_cents::bigint, cap_cents), 0)::integer;
  END IF;
