...
async function fetchStooq(symbol: string): Promise<{ open: number; close: number } | null> {
  try {
    const res = await fetch(`https://stooq.com/q/l/?s=${symbol}&f=sd2t2ohlc&h&e=csv`, {
  ...
  const assets: AssetPrice[] = [
    gold
      ? {
          ...
          name: "Gold",
          price: gold.close,
          change: ((gold.close - gold.open) / gold.open) * 100,
        }
    ...
    oil
      ? {
          ...
          name: "Crude Oil",
          price: oil.close,
          change: ((oil.close - oil.open) / oil.open) * 100,
        }
...
export async function loadInvestPrices() {
